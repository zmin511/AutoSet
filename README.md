...
цепочки простых callable-присваиваний, `getattr()` и `__import__()`, а прямым
SQLite opener считаются как `sqlite3.connect`, так и `sqlite3.Connection`.
...
комментариев и CTE, а также записывающие PRAGMA. Непрозрачный SQL в
...
во внутренние writers. Формально существующий, но недостижимый backup или
callback без `create_verified_audio_backup()` не принимается. Post-commit
...
Allowlist задаёт точную пару `файл + функция`, число вызовов и отпечатки
утверждённых SQL-операций; для каждого safe-write дополнительно фиксируются
operation label и callback. Guardrail отклоняет новый вызов, исчезновение
существующего и замену одной разрешённой мутации другой.
