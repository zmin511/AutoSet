...
Разрешение символов учитывает `import`/`from import` отдельно в каждой
лексической области, затенение параметрами и определениями функций, callable в
значениях параметров по умолчанию у функций и lambda, цепочки присваиваний,
распаковку, циклы/comprehension, условные callable и индексируемые контейнеры.
Опасные persistence-callable нельзя передавать как значения: это fail-closed
останавливает `partial`, `map`, контейнерные и пользовательские диспетчеры ещё
до скрытого вызова. Wildcard-import, динамический `getattr`, `vars`,
`attrgetter`, `methodcaller`, `__import__` и другие механизмы рефлексии запрещены
по умолчанию; исключение для динамического чтения задаётся точной парой
`файл + функция` и числом вызовов. Прямым SQLite opener считаются
...
SQL полностью восстанавливается из констант, переменных, f-string,
конкатенации, `%` и `.format()`. Анализатор проверяет все достижимые значения
условных присваиваний. Если хотя бы одна часть не вычисляется статически либо
среди вариантов возможна запись, выражение отклоняется fail-closed: скрытую
мутацию исключить нельзя.
Проверяются `execute`, `executemany`, `executescript`, DML после комментариев и
CTE, `CREATE VIRTUAL TABLE`, `operator.methodcaller`, а также все PRAGMA, кроме
точного списка доказуемо read-only форм. Необходимые production-запросы чтения
с динамическими идентификаторами или списками placeholders допускаются только
точным allowlist по файлу, функции, числу вызовов и SHA-256 отпечатку AST
SQL-выражения.
...
полного нормализованного текста утверждённых SQL-операций; для каждого
safe-write дополнительно фиксируются его импортированное происхождение,
operation label и callback. Отсутствие каждого ожидаемого production-файла
сообщается отдельно и не останавливает проверку остальных файлов. Guardrail
отклоняет новый вызов, исчезновение существующего и изменение колонок,
значений или условий разрешённой мутации.
